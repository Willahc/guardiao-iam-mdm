...
GITHUB_ORG = os.getenv("GITHUB_ORG", "Willahc")
GITHUB_TIMEOUT = 10
...
def adicionar_membro_org(username: str) -> dict:
    ...
    try:
        resp = requests.put(
            f"https://api.github.com/orgs/{GITHUB_ORG}/memberships/{username}",
            headers=_headers(),
            json={"role": "member"},
        ...
        if resp.status_code in (200, 201):
            ...
            logger.info("Membro %s adicionado à org %s (state=%s)", username, GITHUB_ORG, state)
...
def remover_membro_org(username: str) -> dict:
    ...
    try:
        ...
        if resp.status_code == 204:
            logger.info("Membro %s removido da org %s", username, GITHUB_ORG)
            return {"sucesso": True, "mensagem": f"{username} removido da organização {GITHUB_ORG}"}
